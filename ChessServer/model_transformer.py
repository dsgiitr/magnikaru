...
class PositionalEmbedding(nn.Module):
    def __init__(self,num_tokens=64,embed_dim=1,learned=False):
        super(PositionalEmbedding,self).__init__()
        self.learned=learned
        if learned:
            position_number_scaled= torch.tensor([(i+1 )/ (num_tokens ) for i in range(num_tokens)],dtype=torch.float32)
            self.register_buffer("position_number_scaled",
                                 position_number_scaled.unsqueeze(0).unsqueeze(2))
